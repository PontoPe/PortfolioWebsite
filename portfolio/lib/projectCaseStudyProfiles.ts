...
export const projectCaseStudyProfiles: Record<string, ProjectCaseStudyProfile> = {
  ...
  zombiesweb: {
    ...
    status: "Live",
    environment: "Static-first web experience",
    role: "Frontend architect and interaction designer",
    outcome:
      "Turned a dense game universe into a fast, navigable timeline and mobile reference designed for use during active play.",
    ownership: ["Information architecture", "Astro build", "React islands", "Timeline interaction", "Map guides", "Performance"],
    theme: themes.zombies,
    layout: "product-first",
    sectionOrder: ["overview", "evidence", "decisions", "implementation", "operations", "security", "results"],
    metrics: [
      { value: "Static", label: "default page delivery" },
      { value: "Islands", label: "selective React hydration" },
      { value: "2", label: "primary story threads in the timeline" },
      { value: "Mobile", label: "second-screen design target" },
    ],
    problem: [
      "Call of Duty Zombies lore spans maps, timelines, and story branches that are difficult to follow while someone is also playing.",
      "The site pairs a highly interactive Kronorium with static-first map guides so the experience can be visually rich without making every page pay the same JavaScript cost.",
    ],
    constraints: ["Phone or second-monitor use", "Large lore graph", "Fast guide lookup", "Selective hydration", "Fan-project asset boundaries"],
    nodes: [
      ...
      { id: "guides", label: "Map Guides", zone: "trusted", eyebrow: "Fast reference", responsibility: "Presents steps, parts, and locations in a mobile-friendly form.", data: "Static guide content", exposure: "Public pages", failure: "Individual content remains independently routable." },
    ...
    flow: [
      { title: "Player opens a static page", summary: "Astro returns useful HTML before any interactive island runs.", nodeIds: ["fan", "astro"] },
      { title: "Guide stays lightweight", summary: "Reference content needs little or no client hydration.", nodeIds: ["astro", "guides"] },
      { title: "Kronorium hydrates on demand", summary: "React Flow loads where pan, zoom, and graph traversal add real value.", nodeIds: ["astro", "kronorium"] },
      { title: "Story threads remain traceable", summary: "Directional edges show branching and convergence across the lore.", nodeIds: ["kronorium"] },
      { title: "Assets arrive through the build", summary: "Structured content and media remain separate from runtime app state.", nodeIds: ["assets", "astro", "guides"] },
    ],
    threats: [
      ...
      { category: "Usability", title: "Timeline becomes unreadable on mobile", control: "Separate fast guides plus bounded interactive controls", residual: "Very dense lore still benefits from a larger screen.", nodeIds: ["fan", "kronorium", "guides"] },
    ],
    decisions: [
      { title: "Static by default", summary: "Do not make guide content wait for React.", reason: "The site is often opened mid-game on constrained devices.", tradeoff: "Interactive behavior must be isolated deliberately.", revisit: "Only when a route's core value truly requires client state.", nodeIds: ["astro", "guides"] },
      { title: "Hydrate the Kronorium", summary: "Spend JavaScript where graph interaction earns it.", reason: "Lore relationships are easier to understand through pan, zoom, and connected nodes.", tradeoff: "The flagship timeline has a higher interaction cost.", revisit: "As graph size and mobile behavior evolve.", nodeIds: ["kronorium"] },
      { title: "Guides stay independently routable", summary: "A player should reach one answer directly.", reason: "Reference use should not require navigating the full timeline.", tradeoff: "Content organization needs consistent URLs and structure.", revisit: "As new maps and guide formats are added.", nodeIds: ["fan", "guides"] },
    ],
    operations: [
      { question: "What if JavaScript is slow or unavailable?", behavior: "Static pages and guide content remain readable.", signal: "The interactive island does not initialize.", response: "Preserve the static route and provide useful noninteractive context around the graph.", nodeIds: ["astro", "guides", "kronorium"] },
      { question: "What if the lore graph grows?", behavior: "Node and edge density increases visual and runtime pressure.", signal: "Longer interaction startup and crowded navigation.", response: "Partition story eras, lazy-load detail, and keep direct guide routes independent.", nodeIds: ["kronorium", "assets"] },
    ],
    evidence: [
      { label: "Live experience", title: "ZombiesWeb", detail: "The production site demonstrates the timeline and guide experience.", href: "https://zombies-web.vercel.app/", nodeIds: ["fan", "astro", "kronorium", "guides"] },
    ...
    results: ["Static-first delivery keeps reference pages fast", "The Kronorium makes branching lore explorable", "Mobile guides remain directly useful during play"],
    retrospective: {
      worked: ["The architecture spends hydration only where interaction adds meaning.", "Direct guide pages balance the dense timeline."],
      next: ["More graph partitioning", "Performance budgets per island", "Expanded keyboard navigation"],
      demonstrates: ["Frontend architecture", "Information design", "Performance-aware interaction"],
    },
