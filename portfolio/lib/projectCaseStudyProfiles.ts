...
export const projectCaseStudyProfiles: Record<string, ProjectCaseStudyProfile> = {
  ...
  pontosv: {
    ...
    sectionOrder: ["overview", "security", "decisions", "evidence", "implementation", "operations", "results"],
    metrics: [
      { value: "0", label: "inbound ports on the journal workload", note: "Outbound-only via Cloudflare Tunnel; the origin IP isn't in public DNS" },
      { value: "6 / 6", label: "ssh-bruteforce scenarios verified", note: "cscli explain confirmed detection without attacking production sshd" },
      { value: "8 → 5", label: "CRITICAL CVEs on the OJS image", note: "One root-caused and patched same-day; the rest already scoped for the OJS 3.5 upgrade" },
      { value: "0", label: "drift on the first Ansible dry run", note: "ansible-playbook --check --diff reported changed=0 on every task against the live box" },
      { value: "13", label: "restic snapshots, restore-tested", note: "6.8 GB in the B2 repo at 1.74x compression" },
      { value: "26 / 78", label: "OJS submissions / registered users", note: "Real production client: Revista de Direito do Consumidor, BRASILCON" },
      { value: "3 → 1", label: "network owners after the fix", note: "Two racing DHCP clients and three overlapping firewall managers, reduced to one of each" },
      { value: "0", label: "recurrences since the architectural fix" },
    ],
    problem: [
    ...
    threats: [
      { category: "Vulnerability", title: "Unpatched CRITICAL in a running container image", control: "Trivy scans weekly (pontosv-check-trivy.timer) and diffs CRITICAL CVE IDs against the last run instead of alerting on raw totals; a real CRITICAL, CVE-2026-31789 (an OpenSSL heap overflow), was found and patched the same day.", residual: "Five CRITICALs remain, tied to the OJS base image; closing them needs the already-scoped OJS 3.5 upgrade, not another scan.", nodeIds: ["docker"] },
      { category: "Drift", title: "Manual changes silently diverging from documented config", control: "An Ansible role now codifies the monitoring and backup stack; ansible-playbook --check --diff against the live box reports changed=0, i.e. drift is actually checked, not assumed.", residual: "Only the monitoring/backup stack is codified so far - OJS, Nextcloud, TeamSpeak, and network config remain hand-managed and could still drift silently.", nodeIds: ["host"] },
      ...
      { category: "Identity", title: "Password-based SSH attack", control: "Key-only SSH with password authentication disabled, plus CrowdSec detection verified two ways: a real SSH session traced end-to-end through the pipeline, and cscli explain confirming all six ssh-bruteforce scenarios match synthetic attack-shaped input.", residual: "No bouncer installed yet, so CrowdSec detects and would alert but doesn't yet auto-block; private-key protection and endpoint security remain necessary.", nodeIds: ["wan", "host"] },
    ...
    decisions: [
      { title: "Zero inbound ports for the highest-value workload", summary: "The OJS journal is reachable only outbound, through a Cloudflare Tunnel; its origin IP isn't in public DNS.", reason: "The journal is the workload an outage or compromise would hurt most - a national academic institution's publication - so it gets the strictest exposure rule on the box: there is nothing to scan.", tradeoff: "Availability now depends on the tunnel provider staying up, not just the host.", revisit: "If a future workload needs inbound access no tunnel can provide.", nodeIds: ["wan", "host"] },
      { title: "Separate the collector from the public reader", summary: "A root-run collector on a timer gathers privileged data; the internet-facing dashboard process only ever reads one JSON snapshot it wrote.", reason: "The public status dashboard needs data that requires root to gather (Docker, journald, cscli, restic), but the process answering public requests has no reason to hold that privilege itself.", tradeoff: "Status data is only as fresh as the last collector run, not live.", revisit: "If the dashboard ever needs to trigger actions rather than just report state.", nodeIds: ["host", "docker"] },
    ...
    operations: [
      { question: "Why did the internet disappear every few weeks, and why did earlier firewall fixes fail?", behavior: "Two racing DHCP clients and a rogue lease repeatedly replaced the default route, while firewalld, ufw, iptables-persistent, and Docker each affected an overlapping layer - so a fix in one tool often wasn't the layer actually blocking traffic.", signal: "journald recorded route changes at the moment of failure; rules changed in one tool while another tool or runtime remained authoritative.", response: "Reduce to one authoritative DHCP client and one firewall authority, with Docker keeping its own chains. Zero recurrences since.", nodeIds: ["router", "host", "docker"] },
    ],
    evidence: [
      { label: "Public evidence", title: "Live status dashboard — sv.pegradowski.com", detail: "Publicly reachable right now: real container health, security posture (CrowdSec/Trivy/SSH), a node-graph of backup history built from actual restic diff output, and system stats. A reader can open it directly instead of taking the write-up's word for it.", href: "https://sv.pegradowski.com", nodeIds: ["host"] },
      { label: "Detection evidence", title: "CrowdSec verified two ways", detail: "A real SSH connection traced end-to-end through the pipeline, and cscli explain confirmed all six ssh-bruteforce scenarios match an attack-shaped synthetic log line without needing to attack production sshd. Runs via a journalctl_filter acquisition since this box has no /var/log/auth.log — everything's in journald.", nodeIds: ["host"] },
      { label: "Vulnerability evidence", title: "CVE-2026-31789 found and patched same-day", detail: "Weekly Trivy scan (pontosv-check-trivy.timer) diffs CRITICAL CVE IDs against the last run instead of alerting on raw totals. A real CRITICAL, an OpenSSL heap overflow, was found and patched the same day — CRITICAL count measured 8 to 5 on the next scan, not claimed.", nodeIds: ["docker"] },
      { label: "IaC evidence", title: "Ansible role proven idempotent on production", detail: "ansible-playbook --check --diff against the live box reported changed=0 on every task — drift detection actually running, not aspirational.", nodeIds: ["host"] },
  ...
  truststack: {
    ...
    evidence: [
      ...
      { label: "PontoAntiCrack · the detonation", title: "detonation-sg-open.md", detail: "A real attack technique executed with Stratus Red Team against a fully green test suite, and the two defects it exposed that a green suite could not - 174 tests passing at the first, 176 at the second.", href: "https://github.com/PontoPe/PontoAntiCrack/blob/main/docs/evidence/detonation-sg-open.md", nodeIds: ["detect", "respond"] },
  ...
  portfolio: {
    ...
    status: "Live",
    environment: "Next.js static export · GitHub Actions · FTP host",
    role: "Designer, frontend engineer, and pipeline owner",
    outcome:
    ...
    ownership: ["Visual system", "Next.js architecture", "Content pipeline", "CI/CD workflow", "Interactive terminal", "Project content", "Accessibility"],
    theme: themes.portfolio,
    layout: "evidence-first",
    sectionOrder: ["overview", "decisions", "implementation", "evidence", "operations", "security", "results"],
    metrics: [
      { value: "2", label: "repositories composed per build", note: "Public site plus the private Obsidian vault" },
      { value: "00:17", label: "nightly rebuild for scheduled posts", note: "America/Sao_Paulo, so a dated note publishes itself" },
      { value: "Static", label: "export deployed over FTP", note: "No server runtime to operate or patch" },
      { value: "11", label: "terminal commands over a virtual filesystem" },
    ],
    problem: [
    ...
    constraints: ["Static-only hosting over FTP", "Private writing vault", "Publishing must cost one commit", "Scheduled posts without a server", "Fast first read", "Keyboard and mobile access", "No sensitive production details"],
    nodes: [
      ...
      { id: "terminal", label: "PontoPe OS", zone: "trusted", eyebrow: "Client-side island", responsibility: "Runs ls, cd, cat, tab completion, and shortcuts over a virtual filesystem.", data: "Device-local session state", exposure: "Browser only, no shell and no network calls", failure: "The rest of the page keeps working without it." },
      { id: "next", label: "Static Export", zone: "trusted", eyebrow: "Next.js build and published artifact", responsibility: "Renders case-study routes from typed profiles and blog routes from markdown, then ships out/ as plain files over FTP.", data: "Project profiles, frontmatter, and page metadata", exposure: "Build-time render, public static files", failure: "A content or type error fails the build, so the previous deploy stays online untouched." },
      { id: "actions", label: "Deploy Workflow", zone: "internal", eyebrow: "GitHub Actions", responsibility: "Composes the site and the private vault, builds, and uploads the artifact.", data: "Scoped vault token and FTP credentials", exposure: "Ephemeral runner with read-only repository permissions", failure: "A failed job publishes nothing; the live site is untouched." },
      { id: "vault", label: "Obsidian Vault", zone: "internal", eyebrow: "Private git CMS", responsibility: "Holds posts as markdown notes with frontmatter, images included.", data: "Private writing, drafts, and scheduled posts", exposure: "Private repository, never deployed as-is", failure: "If the checkout fails the site builds with no blog posts instead of stale ones." },
    ],
    flow: [
      { title: "A note becomes a post", summary: "Writing happens in Obsidian; frontmatter carries title, description, and an ISO date.", nodeIds: ["vault"] },
      { title: "The vault dispatches a build", summary: "A push in the private repository fires repository_dispatch at the website repository.", nodeIds: ["vault", "actions"] },
      { title: "Two repositories become one tree", summary: "The workflow checks the vault into portfolio/_content with a scoped token and copies its images into public/.", nodeIds: ["actions", "next"] },
      { title: "Export or publish nothing", summary: "next build emits static routes; future-dated posts are excluded and a failed build never reaches the host.", nodeIds: ["next"] },
      { title: "Nightly rebuild releases the schedule", summary: "A 00:17 America/Sao_Paulo cron re-exports the site so dated posts appear without a commit.", nodeIds: ["actions", "visitor"] },
    ],
    threats: [
      { category: "Supply chain", title: "A third-party action could read job secrets", control: "Every action pinned to a full commit SHA, permissions: contents: read, and persist-credentials disabled on both checkouts", residual: "The vault token still grants read access to private content while the job runs, so it needs periodic rotation.", nodeIds: ["actions", "vault"] },
      { category: "Confidentiality", title: "Private notes leak into a public build", control: "Only markdown in the vault's posts directory is rendered, images are copied by explicit extension, and future-dated notes are excluded from the export", residual: "Anything committed to the posts directory is publishable by definition; review happens before the commit, not after.", nodeIds: ["vault", "next"] },
      { category: "Credentials", title: "Deploy credentials sit in the pipeline", control: "FTP host, user, and password live only as repository secrets and never enter the repository or the exported artifact", residual: "The host authenticates by password; key-based transport would be a real improvement.", nodeIds: ["actions", "next"] },
      { category: "Client trust", title: "The terminal invites people to poke at it", control: "The filesystem is a static object with no shell, no eval, and no network calls, so commands only read shipped strings", residual: "Anything placed in that filesystem is public content, easter eggs included.", nodeIds: ["terminal", "visitor"] },
    ],
    decisions: [
      { title: "Obsidian is the CMS", summary: "Publishing means committing a note in a private vault repository.", reason: "A blog only survives if writing and publishing cost nothing beyond writing.", tradeoff: "The site cannot build a complete blog without access to a second, private repository.", revisit: "If posts ever need richer structure than markdown frontmatter.", nodeIds: ["vault", "actions"] },
      { title: "Static export over a server runtime", summary: "output: export plus trailingSlash, deployed as files over FTP.", reason: "No runtime means nothing to patch, scale, or leave unpatched on a shared host.", tradeoff: "Anything dynamic has to become either a build step or a client-side island.", revisit: "Only if content genuinely requires per-request rendering.", nodeIds: ["next", "visitor"] },
      { title: "Schedule by rebuilding, not by hiding", summary: "Future-dated posts are absent from generateStaticParams, with dynamicParams disabled, and a nightly cron re-exports the site.", reason: "A static host cannot notice that the date changed, and hiding a route client-side is not publishing control.", tradeoff: "Publication granularity is a day, not an hour, and it depends on the scheduled workflow actually running.", revisit: "If a post ever needs to land at a specific time.", nodeIds: ["next", "actions"] },
      { title: "The terminal is an island, not the site", summary: "PontoPe OS is progressive enhancement over ordinary links and pages.", reason: "The identity of the site should never be a prerequisite for reading it.", tradeoff: "Two navigation models have to stay consistent with each other.", revisit: "If the terminal grows commands that touch anything but local state.", nodeIds: ["terminal", "visitor"] },
      { title: "One system, many identities", summary: "Case studies reuse behavior while varying visual and narrative emphasis.", reason: "Consistency builds trust, but identical pages flatten the work.", tradeoff: "The content model needs disciplined project-specific profiles.", revisit: "As new project families expose missing components.", nodeIds: ["next", "visitor"] },
      { title: "Sanitize before publishing", summary: "Redaction is a content operation, not a CSS effect.", reason: "A security portfolio cannot leak what it claims to protect.", tradeoff: "Some evidence must remain summarized instead of shown.", revisit: "For every artifact independently.", nodeIds: ["vault", "next"] },
    ],
    operations: [
      { question: "What if the vault push does not trigger a build?", behavior: "The post waits, and the site keeps serving the previous deploy.", signal: "No workflow run appears after the dispatch event.", response: "The nightly cron picks the post up anyway, and workflow_dispatch forces it immediately.", nodeIds: ["vault", "actions"] },
      { question: "What if the private content checkout fails?", behavior: "The build continues and produces a site with no blog posts rather than a broken deploy.", signal: "The workflow log reports a missing _content directory and the blog index comes out empty.", response: "Rotate or repair the vault token, then re-run the job before the next scheduled rebuild.", nodeIds: ["actions", "vault", "next"] },
      { question: "What if a post is dated in the future?", behavior: "It is excluded from the index and from the generated routes, so the URL 404s until its date.", signal: "The route is missing from the export while the note exists in the vault.", response: "Nothing: the nightly rebuild publishes it on the date the frontmatter asked for.", nodeIds: ["next", "actions"] },
      { question: "What if a project profile is incomplete?", behavior: "Typed build validation surfaces missing required content.", signal: "The production build fails before the route is published.", response: "Add verified content or deliberately omit the unsupported tool.", nodeIds: ["next", "actions"] },
    ],
    evidence: [
      { label: "Live artifact", title: "The portfolio itself", detail: "The current page is both the delivery surface and a demonstration of the system.", nodeIds: ["visitor", "next"] },
      { label: "Pipeline", title: "The deploy workflow", detail: "Shows the dual checkout, SHA-pinned actions, read-only permissions, and the cron that releases scheduled posts.", nodeIds: ["actions", "vault", "next"] },
      { label: "Content model", title: "Frontmatter to published route", detail: "Date normalization, timezone-aware publication checks, and static params derived only from published posts.", nodeIds: ["vault", "next"] },
      { label: "Interaction", title: "Terminal and case-study tools", detail: "Demonstrate keyboard-accessible, device-local interfaces without external state.", nodeIds: ["terminal", "visitor"] },
    ],
    results: [
    ...
    retrospective: {
      worked: ["Making the writing tool the CMS removed the reason blogs go stale.", "A nightly rebuild turned scheduling into a solved problem on static hosting.", "The terminal identity carries across very different project families."],
      next: ["Move deployment to key-based transport instead of FTP passwords", "Automated link and markdown checks before the build ships", "Automated visual regression checks and per-project social cards"],
      demonstrates: ["CI/CD and secure pipeline design", "Content architecture", "Design systems", "Accessible frontend engineering"],
    },
